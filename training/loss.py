...
class Losses():
    ...
    #     l1 = tf.reduce_mean(((sa-sa1) - (sab-sab1))**2+((sab2-sab) - (sa2-sa))**2+((sa1-sa2) - (sab1-sab2))**2)
    #     l2 = tf.reduce_mean(tf.reduce_sum(-(tf.nn.l2_normalize(sa-sa1,
    #                         axis=[-1]) * tf.nn.l2_normalize(sab-sab1, axis=[-1]) * tf.nn.l2_normalize(sab1-sab2,axis=[-1])), axis=-1))
    #     return l1+l2
    ...
    # def loss_siamese(self, sa, sa1,sa2):
    #     logits = tf.sqrt(tf.reduce_sum(((sa-sa1)**2+(sa1-sa2)**2+(sa2-sa)**2), axis=-1, keepdims=True))
